    ...
    ReentrancyGuardUpgradeable
{
    ...
    error CumulativeAmountExceedsDeposit(bytes32 channelId, uint256 cumulativeAmount, uint256 deposit);
    ...
    event ChannelClosed(bytes32 indexed channelId, uint256 cumulativeAmount, uint256 payout, uint256 refund);
    ...
    function initialize(address owner_, uint64 closeDelaySeconds_) external initializer {
        if (owner_ == address(0)) {
    ...
    function setCloseDelaySeconds(uint64 nextCloseDelaySeconds) external onlyOwner {
    ...
    function close(
        ...
        if (payout > 0) {
            IERC20(channel.token).safeTransfer(channel.payee, payout);
        }
        if (refund > 0) {
    ...
    function getChannelsBatch(bytes32[] calldata channelIds) external view returns (Channel[] memory channels) {
    ...
    function computeChannelId(
        ...
        return keccak256(abi.encode(block.chainid, address(this), payer, payee, token, authorizedSigner, salt));
    }

    function getVoucherDigest(bytes32 channelId, uint256 cumulativeAmount) public view returns (bytes32) {
    ...
    function _requireActiveChannel(bytes32 channelId) internal view returns (Channel storage channel) {
    ...
    function _validateVoucher(
        ...
        address recoveredSigner = ECDSA.recover(getVoucherDigest(channelId, cumulativeAmount), signature);
